...
| 優先 | 項目 | 說明 |
| :--: | :--- | :--- |
| 1 | **Mel 頻帶數 128 → 160** | 提升頻率解析度，對鳥鳴細節有幫助；需重新前處理 HDF5 |
| 2 | **FFT window 調整（n_fft 1024 → 512）** | 提升時間解析度，BirdNET 論文建議短 window；需重新前處理 |
| 3 | **SpecAugment（頻率/時間遮罩）** | 防止過擬合，實作簡單，在訓練迴圈中動態套用，不需重新前處理 |

> 注意：項目 1 和 2 都需要重新跑 `preprocess_waveform.py` 重建 HDF5，建議一次調整後一起重跑。
